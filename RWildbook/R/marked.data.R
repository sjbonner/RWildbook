...
#'
#' The \code{marked.data} function format the wildbook data set that users search with
...
#' data set with \code{process.data} function in \code{marked} package. The \code{marked.data}
...
#' The class of the ouput of \code{marked.data} is "data.table" and "data.frame". With installing
...
#'                           
#' ## Remove 0 histories
#' zeros = which(marked.data1.1$ch==paste0(rep(0,19),collapse=""))
#' marked.data1.1 = marked.data1.1[-zeros,]
#'            
#' ## Fit simple CJS model in marked                                                         
#' marked.data1.proc=process.data(marked.data1.1,model="cjs",begin.time=1)
#' marked.data1.ddl=make.design.data(marked.data1.proc)
#' marked.data1.cjs=crm(marked.data1.proc,marked.data1.ddl,model.parameters=list(Phi=list(formula=~time),p=list(formula=~time)))
#'
